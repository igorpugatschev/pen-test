...
Книги курса использованы автором как источники для построения этой лекции, но не как обязательное домашнее чтение. Студенту не нужно искать недостающую теорию в отдельных файлах или внешних материалах: все понятия, команды, ограничения, безопасные примеры, ожидаемые результаты и критерии сдачи для темы `Урок 38: Shodan и Censys — OSINT разведка` должны быть понятны из текущего урока.
...
### 1. Предмет урока: Урок 38: Shodan и Censys — OSINT разведка

Этот блок учит управлять security-инструментами как частью QA-процесса: режим, цель, скорость, вывод, triage и отчет важнее самого факта запуска. В этом уроке центральная модель: passive internet exposure intelligence, query syntax, asset ownership validation и no-touch OSINT. Студент должен понять ее внутри самого Markdown-файла, без необходимости искать базовую теорию в книгах или внешних статьях.
...
### 3. Ключевые понятия: Shodan и Censys — OSINT разведка

`Target` — конкретная разрешенная цель: локальная папка, localhost, учебная лаборатория, cloud lab или olddev-стенд.

`Mechanism` — технический механизм урока: passive internet exposure intelligence, query syntax, asset ownership validation и no-touch OSINT. Его нужно объяснить до команды или инструмента.
...
### 4. SDET-практика: Shodan и Censys — OSINT разведка
...
### 7. Типичные ошибки: Shodan и Censys — OSINT разведка

Первая ошибка — выполнять практику до понимания модели `passive internet exposure intelligence, query syntax, asset ownership validation и no-touch OSINT`. Это превращает обучение в копирование команд.
...
Студент должен выйти из урока с рабочим пониманием: что такое passive internet exposure intelligence, query syntax, asset ownership validation и no-touch OSINT, как это безопасно проверить, что приложить как evidence и какие действия требуют approval.
...
Лекция считается освоенной, если студент может объяснить `passive internet exposure intelligence, query syntax, asset ownership validation и no-touch OSINT` своими словами, выполнить безопасный путь новичка, получить ожидаемый вывод, интерпретировать его и оформить evidence. Для темы `Урок 38: Shodan и Censys — OSINT разведка` минимальная сдача — оформить воспроизводимый артефакт по модели: passive internet exposure intelligence, query syntax, asset ownership validation и no-touch OSINT, без нарушения ограничения: действия вне scope или без approval.
...
Минимальный эталонный артефакт для сдачи по теме `Урок 38: Shodan и Censys — OSINT разведка`:
