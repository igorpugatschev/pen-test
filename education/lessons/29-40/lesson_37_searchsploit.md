...
Книги курса использованы автором как источники для построения этой лекции, но не как обязательное домашнее чтение. Студенту не нужно искать недостающую теорию в отдельных файлах или внешних материалах: все понятия, команды, ограничения, безопасные примеры, ожидаемые результаты и критерии сдачи для темы `Урок 37: SearchSploit — поиск эксплойтов` должны быть понятны из текущего урока.
...
### 1. Предмет урока: Урок 37: SearchSploit — поиск эксплойтов

Этот блок учит управлять security-инструментами как частью QA-процесса: режим, цель, скорость, вывод, triage и отчет важнее самого факта запуска. В этом уроке центральная модель: local exploit database research, version matching, exploitability review, patch awareness и no-execution product boundary. Студент должен понять ее внутри самого Markdown-файла, без необходимости искать базовую теорию в книгах или внешних статьях.
...
### 2. Модель и границы: SearchSploit — поиск эксплойтов

Модель `local exploit database research, version matching, exploitability review, patch awareness и no-execution product boundary` нужно читать как набор связанных элементов, а не как список слов. В каждом упражнении есть субъект действия, разрешенная цель, технический механизм, наблюдаемый результат и решение: это `observation`, `finding`, `not applicable`, `not reproducible` или `requires approval`.
...
### 3. Ключевые понятия: SearchSploit — поиск эксплойтов

`Target` — конкретная разрешенная цель: локальная папка, localhost, учебная лаборатория, cloud lab или olddev-стенд.

`Mechanism` — технический механизм урока: local exploit database research, version matching, exploitability review, patch awareness и no-execution product boundary. Его нужно объяснить до команды или инструмента.
...
### 4. SDET-практика: SearchSploit — поиск эксплойтов
...
Минимальный результат урока: оформить воспроизводимый артефакт по модели: local exploit database research, version matching, exploitability review, patch awareness и no-execution product boundary. Если результат не достигается, студент должен уметь объяснить, чего не хватает: разрешения, среды, тестовых данных, изученного инструмента или безопасной лаборатории.
...
### 7. Типичные ошибки: SearchSploit — поиск эксплойтов

Первая ошибка — выполнять практику до понимания модели `local exploit database research, version matching, exploitability review, patch awareness и no-execution product boundary`. Это превращает обучение в копирование команд.
...
Студент должен выйти из урока с рабочим пониманием: что такое local exploit database research, version matching, exploitability review, patch awareness и no-execution product boundary, как это безопасно проверить, что приложить как evidence и какие действия требуют approval.
...
Лекция считается освоенной, если студент может объяснить `local exploit database research, version matching, exploitability review, patch awareness и no-execution product boundary` своими словами, выполнить безопасный путь новичка, получить ожидаемый вывод, интерпретировать его и оформить evidence. Для темы `Урок 37: SearchSploit — поиск эксплойтов` минимальная сдача — оформить воспроизводимый артефакт по модели: local exploit database research, version matching, exploitability review, patch awareness и no-execution product boundary, без нарушения ограничения: действия вне scope или без approval.
