...
Книги курса использованы автором как источники для построения этой лекции, но не как обязательное домашнее чтение. Студенту не нужно искать недостающую теорию в отдельных файлах или внешних материалах: все понятия, команды, ограничения, безопасные примеры, ожидаемые результаты и критерии сдачи для темы `Занятие 14. Маршрутизация: таблицы маршрутов, traceroute, route` должны быть понятны из текущего урока.
...
### 1. Предмет урока: Занятие 14. Маршрутизация: таблицы маршрутов, traceroute, route

Этот блок объясняет сетевую основу web-безопасности: где заканчивается DNS, где начинается TCP/TLS/HTTP и как наблюдение превращается в инженерное evidence. В этом уроке центральная модель: routing table, default gateway, traceroute, VPN route, NAT и safe path observation. Студент должен понять ее внутри самого Markdown-файла, без необходимости искать базовую теорию в книгах или внешних статьях.
...
### 2. Модель и границы: таблицы маршрутов, traceroute, route

Модель `routing table, default gateway, traceroute, VPN route, NAT и safe path observation` нужно читать как набор связанных элементов, а не как список слов. В каждом упражнении есть субъект действия, разрешенная цель, технический механизм, наблюдаемый результат и решение: это `observation`, `finding`, `not applicable`, `not reproducible` или `requires approval`.
...
### 3. Ключевые понятия: таблицы маршрутов, traceroute, route

`Target` — конкретная разрешенная цель: локальная папка, localhost, учебная лаборатория, cloud lab или olddev-стенд.

`Mechanism` — технический механизм урока: routing table, default gateway, traceroute, VPN route, NAT и safe path observation. Его нужно объяснить до команды или инструмента.
...
### 4. SDET-практика: таблицы маршрутов, traceroute, route
...
Минимальный результат урока: оформить воспроизводимый артефакт по модели: routing table, default gateway, traceroute, VPN route, NAT и safe path observation. Если результат не достигается, студент должен уметь объяснить, чего не хватает: разрешения, среды, тестовых данных, изученного инструмента или безопасной лаборатории.
...
### 7. Типичные ошибки: таблицы маршрутов, traceroute, route

Первая ошибка — выполнять практику до понимания модели `routing table, default gateway, traceroute, VPN route, NAT и safe path observation`. Это превращает обучение в копирование команд.
...
Студент должен выйти из урока с рабочим пониманием: что такое routing table, default gateway, traceroute, VPN route, NAT и safe path observation, как это безопасно проверить, что приложить как evidence и какие действия требуют approval.
...
Лекция считается освоенной, если студент может объяснить `routing table, default gateway, traceroute, VPN route, NAT и safe path observation` своими словами, выполнить безопасный путь новичка, получить ожидаемый вывод, интерпретировать его и оформить evidence. Для темы `Занятие 14. Маршрутизация: таблицы маршрутов, traceroute, route` минимальная сдача — оформить воспроизводимый артефакт по модели: routing table, default gateway, traceroute, VPN route, NAT и safe path observation, без нарушения ограничения: действия вне scope или без approval.
...
```bash
curl --max-time 10 -v https://olddev.slider-ai.ru 2>&1 | grep -i "trying"
```
...
3. **route vs ip route**: Команда `route -n` устарела, используйте `netstat -rn` для просмотра таблицы маршрутизации.
...
5. **Углубление после урока 29, lab-only**: используйте nmap с различными типами пинга для обнаружения хостов только в изолированной лабораторной сети:
```bash
# lab-only: не выполнять в домашней сети и не выполнять на Slider AI
