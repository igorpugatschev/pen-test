...
Книги курса использованы автором как источники для построения этой лекции, но не как обязательное домашнее чтение. Студенту не нужно искать недостающую теорию в отдельных файлах или внешних материалах: все понятия, команды, ограничения, безопасные примеры, ожидаемые результаты и критерии сдачи для темы `Занятие 10. TCP/IP: стек, IP-адресация, подсети, маски` должны быть понятны из текущего урока.
...
### 4. SDET-практика: стек, IP-адресация, подсети, маски
...
Минимальный результат урока: объяснить, почему конкретный host:port доступен или недоступен без сканирования сети. Если результат не достигается, студент должен уметь объяснить, чего не хватает: разрешения, среды, тестовых данных, изученного инструмента или безопасной лаборатории.
...
### 7. Типичные ошибки: стек, IP-адресация, подсети, маски

Первая ошибка — выполнять практику до понимания модели `IP-адрес, маска, подсеть, gateway, TCP/UDP, порт, connect и timeout`. Это превращает обучение в копирование команд.
...
Студент должен выйти из урока с рабочим пониманием: что такое IP-адрес, маска, подсеть, gateway, TCP/UDP, порт, connect и timeout, как это безопасно проверить, что приложить как evidence и какие действия требуют approval.
...
Лекция считается освоенной, если студент может объяснить `IP-адрес, маска, подсеть, gateway, TCP/UDP, порт, connect и timeout` своими словами, выполнить безопасный путь новичка, получить ожидаемый вывод, интерпретировать его и оформить evidence. Для темы `Занятие 10. TCP/IP: стек, IP-адресация, подсети, маски` минимальная сдача — объяснить, почему конкретный host:port доступен или недоступен без сканирования сети, без нарушения ограничения: сканирование подсетей и UDP sweep до изучения инструментов.
...
3. Зафиксируйте сетевой baseline без обращения к сторонним сервисам:
```bash
ipconfig getifaddr en0 2>/dev/null || ifconfig en0
scutil --dns | head -20
```
...
### Задача 3: Наблюдение TCP без сканирования

В этом уроке `nmap` еще не используется: сначала нужно понять TCP/IP и только потом переходить к инструментам сканирования в уроке 29.

1. Проверьте одиночное TCP-соединение к разрешенному HTTPS-порту olddev:
```bash
nc -vz olddev.slider-ai.ru 443
```

2. Сравните результат с HTTP-level наблюдением:
```bash
curl -I https://olddev.slider-ai.ru
```

3. В evidence объясните разницу: `nc` проверяет TCP connect, а `curl -I` показывает HTTP response headers поверх TLS/HTTP.

### Задача 4: Анализ TCP-handshake

1. Запустите tcpdump только для локального/lab-трафика:
```bash
sudo tcpdump -i lo0 -c 20 'tcp and port 8080' -vvv
```
...
2. **Безопасное наблюдение TCP**: выполните `nc -vz olddev.slider-ai.ru 443` и `curl -I https://olddev.slider-ai.ru`. Объясните, почему это не сканирование диапазона и почему оно укладывается в safe observation.

3. **Анализ соединений на macOS**: выполните `netstat -an | head -30` и для 3 строк определите протокол, локальный адрес, удаленный адрес и состояние TCP. Kali/Linux-вариант `ss -tunap` переносится в углубление.

4. **Работа с маршрутизацией**: Добавьте временный статический маршрут до тестовой сети и проверьте таблицу маршрутизации:
```bash
netstat -rn
route -n get olddev.slider-ai.ru
```
...
- **IPv6 на macOS**: Для просмотра IPv6-адресов используйте `ifconfig` или `ifconfig` на macOS или `ip addr` в Kali/Linux (если установлен iproute2mac: `brew install iproute2mac`).
