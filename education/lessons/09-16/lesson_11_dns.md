...
Книги курса использованы автором как источники для построения этой лекции, но не как обязательное домашнее чтение. Студенту не нужно искать недостающую теорию в отдельных файлах или внешних материалах: все понятия, команды, ограничения, безопасные примеры, ожидаемые результаты и критерии сдачи для темы `Занятие 11. DNS: как работает, записи A, AAAA, CNAME, MX, TXT` должны быть понятны из текущего урока.
...
### 4. SDET-практика: как работает, записи A, AAAA, CNAME, MX, TXT
...
Минимальный результат урока: получить DNS-ответ и объяснить record, TTL и статус. Если результат не достигается, студент должен уметь объяснить, чего не хватает: разрешения, среды, тестовых данных, изученного инструмента или безопасной лаборатории.
...
### 7. Типичные ошибки: как работает, записи A, AAAA, CNAME, MX, TXT

Первая ошибка — выполнять практику до понимания модели `resolver, authoritative DNS, record types A/AAAA/CNAME/MX/TXT, TTL и NXDOMAIN`. Это превращает обучение в копирование команд.
...
Студент должен выйти из урока с рабочим пониманием: что такое resolver, authoritative DNS, record types A/AAAA/CNAME/MX/TXT, TTL и NXDOMAIN, как это безопасно проверить, что приложить как evidence и какие действия требуют approval.
...
Лекция считается освоенной, если студент может объяснить `resolver, authoritative DNS, record types A/AAAA/CNAME/MX/TXT, TTL и NXDOMAIN` своими словами, выполнить безопасный путь новичка, получить ожидаемый вывод, интерпретировать его и оформить evidence. Для темы `Занятие 11. DNS: как работает, записи A, AAAA, CNAME, MX, TXT` минимальная сдача — получить DNS-ответ и объяснить record, TTL и статус, без нарушения ограничения: массовый enum доменов без разрешения.
...
dig olddev.slider-ai.ru MX
dig olddev.slider-ai.ru TXT
dig olddev.slider-ai.ru NS
```

4. Краткий вывод:
```bash
dig +short olddev.slider-ai.ru A
```
...
2. Попробуйте для своего публичного IP (узнайте его через `ipconfig getifaddr en0`)

### Задача 3: Поиск поддоменов через DNS

1. Используйте host для перечисления записей:
```bash
host -t ns olddev.slider-ai.ru
host -t mx olddev.slider-ai.ru
```
...
1. **Анализ DNS-записей разрешенного домена**: Используйте `olddev.slider-ai.ru` или учебный локальный домен только для обычных A/AAAA/CNAME/TXT/NS-запросов. Составьте схему, показывающую, какие записи видны без активного enumeration.
...
```bash
curl -H 'accept: application/dns-json' 'https://cloudflare-dns.com/dns-query?name=olddev.slider-ai.ru&type=A'
```
