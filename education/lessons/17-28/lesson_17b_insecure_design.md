...
Книги курса использованы автором как источники для построения этой лекции, но не как обязательное домашнее чтение. Студенту не нужно искать недостающую теорию в отдельных файлах или внешних материалах: все понятия, команды, ограничения, безопасные примеры, ожидаемые результаты и критерии сдачи для темы `Занятие 17b. OWASP A04: Insecure Design — Небезопасный дизайн` должны быть понятны из текущего урока.
...
### 1. Предмет урока: Занятие 17b. OWASP A04: Insecure Design — Небезопасный дизайн

Этот блок переводит OWASP-риски в безопасные SDET test cases: сначала модель уязвимости, затем lab-only механика, затем продуктовая проверка без разрушительных действий. В этом уроке центральная модель: threat modeling, abuse case, missing control, secure-by-design requirement и acceptance criteria. Студент должен понять ее внутри самого Markdown-файла, без необходимости искать базовую теорию в книгах или внешних статьях.
...
### 2. Модель и границы: Insecure Design — Небезопасный дизайн

Модель `threat modeling, abuse case, missing control, secure-by-design requirement и acceptance criteria` нужно читать как набор связанных элементов, а не как список слов. В каждом упражнении есть субъект действия, разрешенная цель, технический механизм, наблюдаемый результат и решение: это `observation`, `finding`, `not applicable`, `not reproducible` или `requires approval`.
...
### 3. Ключевые понятия: Insecure Design — Небезопасный дизайн

`Target` — конкретная разрешенная цель: локальная папка, localhost, учебная лаборатория, cloud lab или olddev-стенд.

`Mechanism` — технический механизм урока: threat modeling, abuse case, missing control, secure-by-design requirement и acceptance criteria. Его нужно объяснить до команды или инструмента.
...
### 4. SDET-практика: Insecure Design — Небезопасный дизайн
...
Минимальный результат урока: оформить воспроизводимый артефакт по модели: threat modeling, abuse case, missing control, secure-by-design requirement и acceptance criteria. Если результат не достигается, студент должен уметь объяснить, чего не хватает: разрешения, среды, тестовых данных, изученного инструмента или безопасной лаборатории.
...
### 7. Типичные ошибки: Insecure Design — Небезопасный дизайн

Первая ошибка — выполнять практику до понимания модели `threat modeling, abuse case, missing control, secure-by-design requirement и acceptance criteria`. Это превращает обучение в копирование команд.
...
Студент должен выйти из урока с рабочим пониманием: что такое threat modeling, abuse case, missing control, secure-by-design requirement и acceptance criteria, как это безопасно проверить, что приложить как evidence и какие действия требуют approval.
...
Лекция считается освоенной, если студент может объяснить `threat modeling, abuse case, missing control, secure-by-design requirement и acceptance criteria` своими словами, выполнить безопасный путь новичка, получить ожидаемый вывод, интерпретировать его и оформить evidence. Для темы `Занятие 17b. OWASP A04: Insecure Design — Небезопасный дизайн` минимальная сдача — оформить воспроизводимый артефакт по модели: threat modeling, abuse case, missing control, secure-by-design requirement и acceptance criteria, без нарушения ограничения: действия вне scope или без approval.
