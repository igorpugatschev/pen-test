...
Для темы `Занятие 26. SSRF: Server-Side Request Forgery, лабы PortSwigger` ключевая модель — доверительная граница web-приложения. Пользовательский ввод, cookies, headers, URL-параметры, body запроса, server-side integrations и session state не должны автоматически считаться надежными. Уязвимость появляется там, где приложение принимает данные, принимает решение или обращается к другому ресурсу без достаточной проверки.
