...
Книги курса использованы автором как источники для построения этой лекции, но не как обязательное домашнее чтение. Студенту не нужно искать недостающую теорию в отдельных файлах или внешних материалах: все понятия, команды, ограничения, безопасные примеры, ожидаемые результаты и критерии сдачи для темы `Занятие 26. SSRF: Server-Side Request Forgery, лабы PortSwigger` должны быть понятны из текущего урока.
...
### 1. Предмет урока: Занятие 26. SSRF: Server-Side Request Forgery, лабы PortSwigger

Этот блок переводит OWASP-риски в безопасные SDET test cases: сначала модель уязвимости, затем lab-only механика, затем продуктовая проверка без разрушительных действий. В этом уроке центральная модель: server-side request, allowlist, metadata service, internal network и egress control. Студент должен понять ее внутри самого Markdown-файла, без необходимости искать базовую теорию в книгах или внешних статьях.
...
### 4. SDET-практика: Server-Side Request Forgery, лабы PortSwigger
...
Минимальный результат урока: объяснить SSRF-риск на lab-примере и оформить requires approval для продукта. Если результат не достигается, студент должен уметь объяснить, чего не хватает: разрешения, среды, тестовых данных, изученного инструмента или безопасной лаборатории.
...
### 7. Типичные ошибки: Server-Side Request Forgery, лабы PortSwigger

Первая ошибка — выполнять практику до понимания модели `server-side request, allowlist, metadata service, internal network и egress control`. Это превращает обучение в копирование команд.
...
Студент должен выйти из урока с рабочим пониманием: что такое server-side request, allowlist, metadata service, internal network и egress control, как это безопасно проверить, что приложить как evidence и какие действия требуют approval.
...
Лекция считается освоенной, если студент может объяснить `server-side request, allowlist, metadata service, internal network и egress control` своими словами, выполнить безопасный путь новичка, получить ожидаемый вывод, интерпретировать его и оформить evidence. Для темы `Занятие 26. SSRF: Server-Side Request Forgery, лабы PortSwigger` минимальная сдача — объяснить SSRF-риск на lab-примере и оформить requires approval для продукта, без нарушения ограничения: запросы к внутренним адресам или metadata endpoints.
...
Минимальный эталонный артефакт для сдачи по теме `Занятие 26. SSRF: Server-Side Request Forgery, лабы PortSwigger`:
