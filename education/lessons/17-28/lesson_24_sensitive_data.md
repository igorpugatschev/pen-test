...
Этот раздел не является заданием “пойди и найди теорию в книгах”. Книги использованы автором курса для подготовки лекции `Занятие 24. Cryptographic Failures: Перехват трафика, поиск секретов`, а студент получает самодостаточное объяснение в разделах `Source-driven theory` и `Теория`.
...
**Cryptographic Failures (A02:2021)** — это уязвимость, при которой приложение неправильно защищает конфиденциальные данные из-за слабого шифрования или его отсутствия: пароли, данные карт, персональные данные (PII).

### Как происходит утечка

1. **Отсутствие шифрования (HTTP)**: Данные передаются в открытом виде, их можно перехватить (Sniffing)
2. **Слабое шифрование**: Использование устаревших алгоритмов (MD5, SHA1) или отсутствие соли
3. **Неправильное хранение**: Пароли, ключи API, токены лежат в БД как есть (без хеширования или с плохим алгоритмом)
...
6. **Слабые сертификаты**: Использование устаревших протоколов TLS (1.0, 1.1) или самоподписанных сертификатов
...
2. Если трафик не защищен HTTPS (TLS), он видит все данные в открытом виде
3. Если HTTPS есть, но сертификат самоподписанный или слабый (TLS 1.0/1.1) — можно подделать или расшифровать
4. Атакующий получает доступ к файлам с конфигом (`.env`, `config.php`) и извлекает секреты
5. Атакующий использует слабые хеши (MD5, SHA1 без соли) для восстановления паролей
