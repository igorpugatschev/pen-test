...
Попробуйте открыть только в локальной bWAPP: http://127.0.0.1:8082/config.inc.php — если доступно, фиксируйте не секрет, а сам факт exposure и redacted пример.

**Шаг 5: Анализ через Burp Suite**
1. Настройте браузер на прокси Burp `127.0.0.1:8080`. Если `8080` занят, создайте отдельный Burp listener на `127.0.0.1:8084`; не используйте порты DVWA/bWAPP/WebGoat как proxy.
...
1. **Перехват с Wireshark**: Используя Wireshark, перехватите трафик при входе в bWAPP с учебными данными. Найдите POST-запрос, но в evidence оставьте только `<redacted-demo-login>` и `<redacted-demo-password>`, без видимых учетных данных.
...
2. **Поиск конфигов**: В bWAPP попробуйте открыть следующие пути (если доступны):
   ...
   Опишите, какие файлы доступны без авторизации, какие классы данных могли утечь. Сами значения секретов не сохраняйте.

3. **Хеши паролей без внешних сервисов**: используйте встроенную mock lookup table и объясните, почему реальные или учебные хеши нельзя отправлять во внешние сервисы.

| Hash prefix | Алгоритм-кандидат | Mock result | Security QA вывод |
|---|---|---|---|
| `e99a18c4...` | MD5-like | `<demo-password>` | слабый unsalted hash, finding только после owner confirmation |
| `5baa61e4...` | SHA1-like | `<demo-password>` | нужен password storage review |
| `$2y$10$...` | bcrypt-like | not cracked | проверить cost factor и password policy |
