...
Книги курса использованы автором как источники для построения этой лекции, но не как обязательное домашнее чтение. Студенту не нужно искать недостающую теорию в отдельных файлах или внешних материалах: все понятия, команды, ограничения, безопасные примеры, ожидаемые результаты и критерии сдачи для темы `Занятие 25. XXE: XML External Entity, практика на WebGoat` должны быть понятны из текущего урока.
...
### 1. Предмет урока: Занятие 25. XXE: XML External Entity, практика на WebGoat

Этот блок переводит OWASP-риски в безопасные SDET test cases: сначала модель уязвимости, затем lab-only механика, затем продуктовая проверка без разрушительных действий. В этом уроке центральная модель: XML parser, external entity, DTD, file/entity resolution и parser hardening. Студент должен понять ее внутри самого Markdown-файла, без необходимости искать базовую теорию в книгах или внешних статьях.
...
### 4. SDET-практика: XML External Entity, практика на WebGoat
...
Минимальный результат урока: показать lab-only риск и продуктовый safe checklist. Если результат не достигается, студент должен уметь объяснить, чего не хватает: разрешения, среды, тестовых данных, изученного инструмента или безопасной лаборатории.
...
### 7. Типичные ошибки: XML External Entity, практика на WebGoat

Первая ошибка — выполнять практику до понимания модели `XML parser, external entity, DTD, file/entity resolution и parser hardening`. Это превращает обучение в копирование команд.
...
Студент должен выйти из урока с рабочим пониманием: что такое XML parser, external entity, DTD, file/entity resolution и parser hardening, как это безопасно проверить, что приложить как evidence и какие действия требуют approval.
...
Лекция считается освоенной, если студент может объяснить `XML parser, external entity, DTD, file/entity resolution и parser hardening` своими словами, выполнить безопасный путь новичка, получить ожидаемый вывод, интерпретировать его и оформить evidence. Для темы `Занятие 25. XXE: XML External Entity, практика на WebGoat` минимальная сдача — показать lab-only риск и продуктовый safe checklist, без нарушения ограничения: чтение файлов или SSRF через XML на продукте.
...
curl http://127.0.0.1:4444/xxe.dtd
