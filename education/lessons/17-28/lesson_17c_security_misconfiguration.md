...
Книги курса использованы автором как источники для построения этой лекции, но не как обязательное домашнее чтение. Студенту не нужно искать недостающую теорию в отдельных файлах или внешних материалах: все понятия, команды, ограничения, безопасные примеры, ожидаемые результаты и критерии сдачи для темы `Занятие 17c. OWASP A05: Security Misconfiguration — Неправильная конфигурация` должны быть понятны из текущего урока.
...
### 1. Предмет урока: Занятие 17c. OWASP A05: Security Misconfiguration — Неправильная конфигурация

Этот блок переводит OWASP-риски в безопасные SDET test cases: сначала модель уязвимости, затем lab-only механика, затем продуктовая проверка без разрушительных действий. В этом уроке центральная модель: security baseline, default settings, exposed debug/config, headers, directory listing и hardening checklist. Студент должен понять ее внутри самого Markdown-файла, без необходимости искать базовую теорию в книгах или внешних статьях.
...
### 2. Модель и границы: Security Misconfiguration — Неправильная конфигурация

Модель `security baseline, default settings, exposed debug/config, headers, directory listing и hardening checklist` нужно читать как набор связанных элементов, а не как список слов. В каждом упражнении есть субъект действия, разрешенная цель, технический механизм, наблюдаемый результат и решение: это `observation`, `finding`, `not applicable`, `not reproducible` или `requires approval`.
...
### 3. Ключевые понятия: Security Misconfiguration — Неправильная конфигурация

`Target` — конкретная разрешенная цель: локальная папка, localhost, учебная лаборатория, cloud lab или olddev-стенд.

`Mechanism` — технический механизм урока: security baseline, default settings, exposed debug/config, headers, directory listing и hardening checklist. Его нужно объяснить до команды или инструмента.
...
### 4. SDET-практика: Security Misconfiguration — Неправильная конфигурация
...
### 7. Типичные ошибки: Security Misconfiguration — Неправильная конфигурация

Первая ошибка — выполнять практику до понимания модели `security baseline, default settings, exposed debug/config, headers, directory listing и hardening checklist`. Это превращает обучение в копирование команд.
...
Студент должен выйти из урока с рабочим пониманием: что такое security baseline, default settings, exposed debug/config, headers, directory listing и hardening checklist, как это безопасно проверить, что приложить как evidence и какие действия требуют approval.
...
Лекция считается освоенной, если студент может объяснить `security baseline, default settings, exposed debug/config, headers, directory listing и hardening checklist` своими словами, выполнить безопасный путь новичка, получить ожидаемый вывод, интерпретировать его и оформить evidence. Для темы `Занятие 17c. OWASP A05: Security Misconfiguration — Неправильная конфигурация` минимальная сдача — оформить воспроизводимый артефакт по модели: security baseline, default settings, exposed debug/config, headers, directory listing и hardening checklist, без нарушения ограничения: действия вне scope или без approval.
...
1. **Оставлять дефолтные пароли** — самая частая ошибка (известные demo-учетки в лаборатории)
...
Минимальный эталонный артефакт для сдачи по теме `Занятие 17c. OWASP A05: Security Misconfiguration — Неправильная конфигурация`:
