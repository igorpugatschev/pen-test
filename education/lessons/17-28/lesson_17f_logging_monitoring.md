...
Книги курса использованы автором как источники для построения этой лекции, но не как обязательное домашнее чтение. Студенту не нужно искать недостающую теорию в отдельных файлах или внешних материалах: все понятия, команды, ограничения, безопасные примеры, ожидаемые результаты и критерии сдачи для темы `Занятие 17f. OWASP A09: Security Logging and Monitoring Failures — Отсутствие логирования и мониторинга` должны быть понятны из текущего урока.
...
### 1. Предмет урока: Занятие 17f. OWASP A09: Security Logging and Monitoring Failures — Отсутствие логирования и мониторинга

Этот блок переводит OWASP-риски в безопасные SDET test cases: сначала модель уязвимости, затем lab-only механика, затем продуктовая проверка без разрушительных действий. В этом уроке центральная модель: audit event, detection signal, missing log, sensitive log data и incident evidence. Студент должен понять ее внутри самого Markdown-файла, без необходимости искать базовую теорию в книгах или внешних статьях.
...
### 2. Модель и границы: Security Logging and Monitoring Failures — Отсутствие логирования и мо

Модель `audit event, detection signal, missing log, sensitive log data и incident evidence` нужно читать как набор связанных элементов, а не как список слов. В каждом упражнении есть субъект действия, разрешенная цель, технический механизм, наблюдаемый результат и решение: это `observation`, `finding`, `not applicable`, `not reproducible` или `requires approval`.
...
### 3. Ключевые понятия: Security Logging and Monitoring Failures — Отсутствие логирования и мо

`Target` — конкретная разрешенная цель: локальная папка, localhost, учебная лаборатория, cloud lab или olddev-стенд.

`Mechanism` — технический механизм урока: audit event, detection signal, missing log, sensitive log data и incident evidence. Его нужно объяснить до команды или инструмента.
...
### 4. SDET-практика: Security Logging and Monitoring Failures — Отсутствие логирования и мо
...
Минимальный результат урока: оформить воспроизводимый артефакт по модели: audit event, detection signal, missing log, sensitive log data и incident evidence. Если результат не достигается, студент должен уметь объяснить, чего не хватает: разрешения, среды, тестовых данных, изученного инструмента или безопасной лаборатории.
...
### 7. Типичные ошибки: Security Logging and Monitoring Failures — Отсутствие логирования и мо

Первая ошибка — выполнять практику до понимания модели `audit event, detection signal, missing log, sensitive log data и incident evidence`. Это превращает обучение в копирование команд.
...
Студент должен выйти из урока с рабочим пониманием: что такое audit event, detection signal, missing log, sensitive log data и incident evidence, как это безопасно проверить, что приложить как evidence и какие действия требуют approval.
...
Лекция считается освоенной, если студент может объяснить `audit event, detection signal, missing log, sensitive log data и incident evidence` своими словами, выполнить безопасный путь новичка, получить ожидаемый вывод, интерпретировать его и оформить evidence. Для темы `Занятие 17f. OWASP A09: Security Logging and Monitoring Failures — Отсутствие логирования и мониторинга` минимальная сдача — оформить воспроизводимый артефакт по модели: audit event, detection signal, missing log, sensitive log data и incident evidence, без нарушения ограничения: действия вне scope или без approval.
...
2. Отправьте 3-5 учебных запросов с неверным паролем только в DVWA, вручную или через Repeater. Это не brute force, а low-rate проверка реакции.
...
```
http://127.0.0.1:8082/sqli_1.php?title=1' OR '1'='1
```
...
# Тяжелый ELK-стек не является обязательным на MacBook Air M2 8GB.
# Для базового пути используйте sample log и logger; ELK/SIEM - только cloud/managed demo.
...
2. **Low-rate проверка реакции**: Используя Repeater или ручной вход, отправьте 3-5 неверных попыток логина в DVWA. Сработала ли какая-то защита? Появилось ли уведомление? Напишите, почему отсутствие реакции — это проблема A09. Intruder и массовые запросы не нужны для сдачи.
...
4. **Сравнение по встроенной матрице**: заполните таблицу ниже, не обращаясь к внешним CMS-документам.

| Система | События безопасности | Где обычно искать | Риск плохого логирования |
|---|---|---|---|
| Web app auth | login success/fail, logout, password reset | app log / audit log | credential attack остается незамеченным |
| API gateway | 401/403/429, schema validation fail | gateway log / SIEM | authorization issue теряется в access log |
| Admin panel | role change, export, deletion | audit log | нельзя расследовать tampering |
| CI/CD | secret scan fail, deploy, dependency alert | pipeline log | vulnerable build проходит в окружение |
