...
Книги курса использованы автором как источники для построения этой лекции, но не как обязательное домашнее чтение. Студенту не нужно искать недостающую теорию в отдельных файлах или внешних материалах: все понятия, команды, ограничения, безопасные примеры, ожидаемые результаты и критерии сдачи для темы `Занятие 23. Broken Authentication: Слабые пароли, Session Fixation` должны быть понятны из текущего урока.
...
### 1. Предмет урока: Занятие 23. Broken Authentication: Слабые пароли, Session Fixation

Этот блок переводит OWASP-риски в безопасные SDET test cases: сначала модель уязвимости, затем lab-only механика, затем продуктовая проверка без разрушительных действий. В этом уроке центральная модель: identity, authentication, session, lockout, reset token, MFA и rate limit. Студент должен понять ее внутри самого Markdown-файла, без необходимости искать базовую теорию в книгах или внешних статьях.
...
### 4. SDET-практика: Слабые пароли, Session Fixation
...
Минимальный результат урока: описать safe auth test без перебора паролей. Если результат не достигается, студент должен уметь объяснить, чего не хватает: разрешения, среды, тестовых данных, изученного инструмента или безопасной лаборатории.
...
### 7. Типичные ошибки: Слабые пароли, Session Fixation

Первая ошибка — выполнять практику до понимания модели `identity, authentication, session, lockout, reset token, MFA и rate limit`. Это превращает обучение в копирование команд.
...
Студент должен выйти из урока с рабочим пониманием: что такое identity, authentication, session, lockout, reset token, MFA и rate limit, как это безопасно проверить, что приложить как evidence и какие действия требуют approval.
...
Лекция считается освоенной, если студент может объяснить `identity, authentication, session, lockout, reset token, MFA и rate limit` своими словами, выполнить безопасный путь новичка, получить ожидаемый вывод, интерпретировать его и оформить evidence. Для темы `Занятие 23. Broken Authentication: Слабые пароли, Session Fixation` минимальная сдача — описать safe auth test без перебора паролей, без нарушения ограничения: brute force, credential stuffing, password spraying.
