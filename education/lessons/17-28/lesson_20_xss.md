...
Для темы `Занятие 20. XSS (Reflected/Stored): Практика на DVWA` ключевая модель — доверительная граница web-приложения. Пользовательский ввод, cookies, headers, URL-параметры, body запроса, server-side integrations и session state не должны автоматически считаться надежными. Уязвимость появляется там, где приложение принимает данные, принимает решение или обращается к другому ресурсу без достаточной проверки.
