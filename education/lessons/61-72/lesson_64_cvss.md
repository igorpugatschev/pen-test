...
### Использование CVSS без внешнего калькулятора

Для обязательного пути используйте встроенную модель CVSS v3.1: `Attack Vector`, `Attack Complexity`, `Privileges Required`, `User Interaction`, `Scope`, `Confidentiality`, `Integrity`, `Availability`. Онлайн-калькуляторы можно открыть как углубление, но оценка в этом уроке должна быть понятна по таблицам и примерам внутри лекции.
