...
Книги курса использованы автором как источники для построения этой лекции, но не как обязательное домашнее чтение. Студенту не нужно искать недостающую теорию в отдельных файлах или внешних материалах: все понятия, команды, ограничения, безопасные примеры, ожидаемые результаты и критерии сдачи для темы `Занятие 45: Passive domain inventory boundaries` должны быть понятны из текущего урока.
...
### 1. Предмет урока: Занятие 45: Passive domain inventory boundaries

Этот блок делает из Python не exploit shortcut, а поддерживаемую security automation: allowlist, dry-run, timeout, structured output и тестируемая архитектура обязательны. В этом уроке центральная модель: passive domain inventory, resolver limits, allowlist, deduplication и no brute-force product boundary. Студент должен понять ее внутри самого Markdown-файла, без необходимости искать базовую теорию в книгах или внешних статьях.
...
### 2. Модель и границы: Passive domain inventory boundaries

Модель `passive domain inventory, resolver limits, allowlist, deduplication и no brute-force product boundary` нужно читать как набор связанных элементов, а не как список слов. В каждом упражнении есть субъект действия, разрешенная цель, технический механизм, наблюдаемый результат и решение: это `observation`, `finding`, `not applicable`, `not reproducible` или `requires approval`.
...
### 3. Ключевые понятия: Passive domain inventory boundaries

`Target` — конкретная разрешенная цель: локальная папка, localhost, учебная лаборатория, cloud lab или olddev-стенд.

`Mechanism` — технический механизм урока: passive domain inventory, resolver limits, allowlist, deduplication и no brute-force product boundary. Его нужно объяснить до команды или инструмента.
...
### 4. SDET-практика: Passive domain inventory boundaries
...
Минимальный результат урока: оформить воспроизводимый артефакт по модели: passive domain inventory, resolver limits, allowlist, deduplication и no brute-force product boundary. Если результат не достигается, студент должен уметь объяснить, чего не хватает: разрешения, среды, тестовых данных, изученного инструмента или безопасной лаборатории.
...
### 7. Типичные ошибки: Passive domain inventory boundaries

Первая ошибка — выполнять практику до понимания модели `passive domain inventory, resolver limits, allowlist, deduplication и no brute-force product boundary`. Это превращает обучение в копирование команд.
...
Студент должен выйти из урока с рабочим пониманием: что такое passive domain inventory, resolver limits, allowlist, deduplication и no brute-force product boundary, как это безопасно проверить, что приложить как evidence и какие действия требуют approval.
...
Лекция считается освоенной, если студент может объяснить `passive domain inventory, resolver limits, allowlist, deduplication и no brute-force product boundary` своими словами, выполнить безопасный путь новичка, получить ожидаемый вывод, интерпретировать его и оформить evidence. Для темы `Занятие 45: Passive domain inventory boundaries` минимальная сдача — оформить воспроизводимый артефакт по модели: passive domain inventory, resolver limits, allowlist, deduplication и no brute-force product boundary, без нарушения ограничения: действия вне scope или без approval.
