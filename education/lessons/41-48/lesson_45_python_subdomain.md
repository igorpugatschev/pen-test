# Занятие 45: Passive domain inventory boundaries
...
**Результат занятия:** passive inventory и отказ от активного перебора без approval.
...
Этот раздел не является заданием “пойди и найди теорию в книгах”. Книги использованы автором курса для подготовки лекции `Занятие 45: Passive domain inventory boundaries`, а студент получает самодостаточное объяснение в разделах `Source-driven theory` и `Теория`.
...
def build_domain_inventory(known_domains: list[str]) -> list[dict]:
    return [{"domain": domain, "source": "known-navigation-or-approved-list", "status": "observation"} for domain in known_domains]


def refuse_active_bruteforce(domain: str) -> None:
    raise RuntimeError(f"Active subdomain brute force for {domain} requires written approval and rate limits")
```
...
Минимальный эталонный артефакт для сдачи по теме `Занятие 45: Passive domain inventory boundaries`:
...
**Уровень прогрессии:** Passive inventory
...
Для Slider AI используйте только домены, уже видимые из навигации, документации или согласованного списка.
