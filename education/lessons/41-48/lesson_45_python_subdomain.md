...
**Уровень прогрессии:** Subdomain script boundaries

### Минимум

Добавьте в скрипт запрет активного перебора для `slider-ai.ru` без отдельного флага `--i-have-written-approval`.

### Практика Slider AI

Проверьте, что скрипт завершает работу с понятным сообщением для Slider AI scope.

### Углубление после изучения следующих уроков

После расширения scope используйте только малый словарь и сохраните rate limit.
