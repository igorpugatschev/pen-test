# Занятие 41: Python network safety helpers
...
**Результат занятия:** локальный TCP helper только для `127.0.0.1` и unit tests на отказ вне allowlist.
...
Этот раздел не является заданием “пойди и найди теорию в книгах”. Книги использованы автором курса для подготовки лекции `Занятие 41: Python network safety helpers`, а студент получает самодостаточное объяснение в разделах `Source-driven theory` и `Теория`.
...
Для темы `Занятие 41: Python network safety helpers` Python используется не для “быстрого эксплойта”, а для создания безопасного Security QA helper. Минимальная архитектура helper включает allowlist, dry-run, timeout, ограничение скорости, понятные исключения, structured output и тест. Если код может обратиться к сети, изменить данные или перебрать значения, поведение по умолчанию должно быть консервативным.

Хороший helper отделяет transport/client, business rule, parser и report formatter. Это переносит идеи SDET и Python-архитектуры в безопасность: код можно тестировать, читать, переиспользовать и объяснять. `requests` или `socket` не вызываются “где попало”; они живут за функцией, где проверяются target, timeout и режим. Ошибки не скрываются, а превращаются в понятный status: success, observation, refused_by_policy, timeout, requires_approval.

Для Slider AI Python-автоматизация допускается только в безопасных сценариях: dry-run, одиночный запрос, allowlist, отсутствие brute force, отсутствие destructive payloads, отсутствие сохранения cookies/tokens. Любой helper обязан уметь отказать небезопасной цели и показать этот отказ в тесте.
...
def check_tcp_port(host: str, port: int, timeout: float = 0.5) -> dict:
    ensure_allowed_host(host)
    import socket
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        sock.settimeout(timeout)
        code = sock.connect_ex((host, port))
    return {"host": host, "port": port, "open": code == 0}
```
...
def test_refuses_external_target():
    with pytest.raises(ValueError):
        check_tcp_port("olddev.slider-ai.ru", 443)
```
...
Минимальный эталонный артефакт для сдачи по теме `Занятие 41: Python network safety helpers`:
...
**Уровень прогрессии:** Safe network helper
...
Запустите helper только на `127.0.0.1`; для Slider AI зафиксируйте, почему сетевой scan запрещен без отдельного approval.
