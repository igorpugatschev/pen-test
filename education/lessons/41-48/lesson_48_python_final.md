...
Итог Python-блока — не offensive toolkit, а поддерживаемый Security QA helper. Он наследует SDET-подход: модульность, тесты, безопасные дефолты, понятный output, review и документация для команды.
