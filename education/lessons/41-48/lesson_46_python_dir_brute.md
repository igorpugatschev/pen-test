...
Книги курса использованы автором как источники для построения этой лекции, но не как обязательное домашнее чтение. Студенту не нужно искать недостающую теорию в отдельных файлах или внешних материалах: все понятия, команды, ограничения, безопасные примеры, ожидаемые результаты и критерии сдачи для темы `Занятие 46: URL inventory helper вместо directory brute force` должны быть понятны из текущего урока.
...
### 1. Предмет урока: Занятие 46: URL inventory helper вместо directory brute force

Этот блок делает из Python не exploit shortcut, а поддерживаемую security automation: allowlist, dry-run, timeout, structured output и тестируемая архитектура обязательны. В этом уроке центральная модель: directory helper safety, local target, rate limit, wordlist control, refusal policy и JSON evidence. Студент должен понять ее внутри самого Markdown-файла, без необходимости искать базовую теорию в книгах или внешних статьях.
...
### 2. Модель и границы: URL inventory helper вместо directory brute force

Модель `directory helper safety, local target, rate limit, wordlist control, refusal policy и JSON evidence` нужно читать как набор связанных элементов, а не как список слов. В каждом упражнении есть субъект действия, разрешенная цель, технический механизм, наблюдаемый результат и решение: это `observation`, `finding`, `not applicable`, `not reproducible` или `requires approval`.
...
### 3. Ключевые понятия: URL inventory helper вместо directory brute force

`Target` — конкретная разрешенная цель: локальная папка, localhost, учебная лаборатория, cloud lab или olddev-стенд.

`Mechanism` — технический механизм урока: directory helper safety, local target, rate limit, wordlist control, refusal policy и JSON evidence. Его нужно объяснить до команды или инструмента.
...
### 4. SDET-практика: URL inventory helper вместо directory brute force
...
Минимальный результат урока: оформить воспроизводимый артефакт по модели: directory helper safety, local target, rate limit, wordlist control, refusal policy и JSON evidence. Если результат не достигается, студент должен уметь объяснить, чего не хватает: разрешения, среды, тестовых данных, изученного инструмента или безопасной лаборатории.
...
### 7. Типичные ошибки: URL inventory helper вместо directory brute force

Первая ошибка — выполнять практику до понимания модели `directory helper safety, local target, rate limit, wordlist control, refusal policy и JSON evidence`. Это превращает обучение в копирование команд.
...
Студент должен выйти из урока с рабочим пониманием: что такое directory helper safety, local target, rate limit, wordlist control, refusal policy и JSON evidence, как это безопасно проверить, что приложить как evidence и какие действия требуют approval.
...
Лекция считается освоенной, если студент может объяснить `directory helper safety, local target, rate limit, wordlist control, refusal policy и JSON evidence` своими словами, выполнить безопасный путь новичка, получить ожидаемый вывод, интерпретировать его и оформить evidence. Для темы `Занятие 46: URL inventory helper вместо directory brute force` минимальная сдача — оформить воспроизводимый артефакт по модели: directory helper safety, local target, rate limit, wordlist control, refusal policy и JSON evidence, без нарушения ограничения: действия вне scope или без approval.
