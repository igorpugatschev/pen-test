...
Книги курса использованы автором как источники для построения этой лекции, но не как обязательное домашнее чтение. Студенту не нужно искать недостающую теорию в отдельных файлах или внешних материалах: все понятия, команды, ограничения, безопасные примеры, ожидаемые результаты и критерии сдачи для темы `Занятие 47: CVE/version mapper` должны быть понятны из текущего урока.
...
### 1. Предмет урока: Занятие 47: CVE/version mapper

Этот блок делает из Python не exploit shortcut, а поддерживаемую security automation: allowlist, dry-run, timeout, structured output и тестируемая архитектура обязательны. В этом уроке центральная модель: CVE, CVSS, CPE, affected version, exploitability, remediation и false positive. Студент должен понять ее внутри самого Markdown-файла, без необходимости искать базовую теорию в книгах или внешних статьях.
...
### 4. SDET-практика: CVE/version mapper
...
Минимальный результат урока: сопоставить версию с advisory без эксплуатации. Если результат не достигается, студент должен уметь объяснить, чего не хватает: разрешения, среды, тестовых данных, изученного инструмента или безопасной лаборатории.
...
### 7. Типичные ошибки: CVE/version mapper

Первая ошибка — выполнять практику до понимания модели `CVE, CVSS, CPE, affected version, exploitability, remediation и false positive`. Это превращает обучение в копирование команд.
...
Студент должен выйти из урока с рабочим пониманием: что такое CVE, CVSS, CPE, affected version, exploitability, remediation и false positive, как это безопасно проверить, что приложить как evidence и какие действия требуют approval.
...
Лекция считается освоенной, если студент может объяснить `CVE, CVSS, CPE, affected version, exploitability, remediation и false positive` своими словами, выполнить безопасный путь новичка, получить ожидаемый вывод, интерпретировать его и оформить evidence. Для темы `Занятие 47: CVE/version mapper` минимальная сдача — сопоставить версию с advisory без эксплуатации, без нарушения ограничения: запуск exploit из CVE без lab.
