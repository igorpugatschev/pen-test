...
Книги курса использованы автором как источники для построения этой лекции, но не как обязательное домашнее чтение. Студенту не нужно искать недостающую теорию в отдельных файлах или внешних материалах: все понятия, команды, ограничения, безопасные примеры, ожидаемые результаты и критерии сдачи для темы `Занятие 44: Nmap XML parser` должны быть понятны из текущего урока.
...
### 1. Предмет урока: Занятие 44: Nmap XML parser

Этот блок делает из Python не exploit shortcut, а поддерживаемую security automation: allowlist, dry-run, timeout, structured output и тестируемая архитектура обязательны. В этом уроке центральная модель: host discovery, TCP connect, port state, service detection, timing, output formats и false positives. Студент должен понять ее внутри самого Markdown-файла, без необходимости искать базовую теорию в книгах или внешних статьях.
...
### 2. Модель и границы: Nmap XML parser
...
### 3. Ключевые понятия: Nmap XML parser
...
### 4. SDET-практика: Nmap XML parser
...
### 7. Типичные ошибки: Nmap XML parser
...
Лекция считается освоенной, если студент может объяснить `host discovery, TCP connect, port state, service detection, timing, output formats и false positives` своими словами, выполнить безопасный путь новичка, получить ожидаемый вывод, интерпретировать его и оформить evidence. Для темы `Занятие 44: Nmap XML parser` минимальная сдача — выполнить localhost/lab scan и классифицировать open port как observation, без нарушения ограничения: subnet scan, -A, -p-, NSE vuln scripts на Slider AI.
