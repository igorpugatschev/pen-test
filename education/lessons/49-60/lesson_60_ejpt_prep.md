# Занятие 60. Подготовка к eJPT: встроенная матрица навыков
...
Книги курса использованы автором как источники для построения этой лекции, но не как обязательное домашнее чтение. Студенту не нужно искать недостающую теорию в отдельных файлах или внешних материалах: все понятия, команды, ограничения, безопасные примеры, ожидаемые результаты и критерии сдачи для темы `Занятие 60. Подготовка к eJPT: встроенная матрица навыков` должны быть понятны из текущего урока.
...
### 1. Предмет урока: Занятие 60. Подготовка к eJPT: встроенная матрица навыков

Этот блок отделяет легальную лабораторную практику от продуктового тестирования: lab дает техническую уверенность, а SDET переносит только безопасный процесс и test design. В этом уроке центральная модель: eJPT lab methodology, eJPT objectives, enumeration discipline, reporting и exam-safe time management. Студент должен понять ее внутри самого Markdown-файла, без необходимости искать базовую теорию в книгах или внешних статьях.
...
### 2. Модель и границы: встроенная матрица eJPT-навыков

Модель `eJPT lab methodology, eJPT objectives, enumeration discipline, reporting и exam-safe time management` нужно читать как набор связанных элементов, а не как список слов. В каждом упражнении есть субъект действия, разрешенная цель, технический механизм, наблюдаемый результат и решение: это `observation`, `finding`, `not applicable`, `not reproducible` или `requires approval`.
...
### 3. Ключевые понятия: встроенная матрица eJPT-навыков

`Target` — конкретная разрешенная цель: локальная папка, localhost, учебная лаборатория, cloud lab или olddev-стенд.

`Mechanism` — технический механизм урока: eJPT lab methodology, eJPT objectives, enumeration discipline, reporting и exam-safe time management. Его нужно объяснить до команды или инструмента.
...
### 4. SDET-практика: встроенная матрица eJPT-навыков
...
### 7. Типичные ошибки: встроенная матрица eJPT-навыков

Первая ошибка — выполнять практику до понимания модели `eJPT lab methodology, eJPT objectives, enumeration discipline, reporting и exam-safe time management`. Это превращает обучение в копирование команд.
...
Студент должен выйти из урока с рабочим пониманием: что такое eJPT lab methodology, eJPT objectives, enumeration discipline, reporting и exam-safe time management, как это безопасно проверить, что приложить как evidence и какие действия требуют approval.
...
Лекция считается освоенной, если студент может объяснить `eJPT lab methodology, eJPT objectives, enumeration discipline, reporting и exam-safe time management` своими словами, выполнить безопасный путь новичка, получить ожидаемый вывод, интерпретировать его и оформить evidence. Для темы `Занятие 60. Подготовка к eJPT: встроенная матрица навыков` минимальная сдача — оформить воспроизводимый артефакт по модели: eJPT lab methodology, eJPT objectives, enumeration discipline, reporting и exam-safe time management, без нарушения ограничения: действия вне scope или без approval.
...
Минимальный эталонный артефакт для сдачи по теме `Занятие 60. Подготовка к eJPT: встроенная матрица навыков`:
