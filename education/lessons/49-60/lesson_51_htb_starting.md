...
Книги курса использованы автором как источники для построения этой лекции, но не как обязательное домашнее чтение. Студенту не нужно искать недостающую теорию в отдельных файлах или внешних материалах: все понятия, команды, ограничения, безопасные примеры, ожидаемые результаты и критерии сдачи для темы `Занятие 51. HackTheBox Starting Point: все машины категории` должны быть понятны из текущего урока.
...
### 1. Предмет урока: Занятие 51. HackTheBox Starting Point: все машины категории

Этот блок отделяет легальную лабораторную практику от продуктового тестирования: lab дает техническую уверенность, а SDET переносит только безопасный процесс и test design. В этом уроке центральная модель: HTB lab access, VPN scope, enumeration notes, write-up evidence и product transfer boundary. Студент должен понять ее внутри самого Markdown-файла, без необходимости искать базовую теорию в книгах или внешних статьях.
...
### 2. Модель и границы: все машины категории

Модель `HTB lab access, VPN scope, enumeration notes, write-up evidence и product transfer boundary` нужно читать как набор связанных элементов, а не как список слов. В каждом упражнении есть субъект действия, разрешенная цель, технический механизм, наблюдаемый результат и решение: это `observation`, `finding`, `not applicable`, `not reproducible` или `requires approval`.
...
### 3. Ключевые понятия: все машины категории

`Target` — конкретная разрешенная цель: локальная папка, localhost, учебная лаборатория, cloud lab или olddev-стенд.

`Mechanism` — технический механизм урока: HTB lab access, VPN scope, enumeration notes, write-up evidence и product transfer boundary. Его нужно объяснить до команды или инструмента.
...
### 4. SDET-практика: все машины категории
...
Минимальный результат урока: оформить воспроизводимый артефакт по модели: HTB lab access, VPN scope, enumeration notes, write-up evidence и product transfer boundary. Если результат не достигается, студент должен уметь объяснить, чего не хватает: разрешения, среды, тестовых данных, изученного инструмента или безопасной лаборатории.
...
### 7. Типичные ошибки: все машины категории

Первая ошибка — выполнять практику до понимания модели `HTB lab access, VPN scope, enumeration notes, write-up evidence и product transfer boundary`. Это превращает обучение в копирование команд.
...
Студент должен выйти из урока с рабочим пониманием: что такое HTB lab access, VPN scope, enumeration notes, write-up evidence и product transfer boundary, как это безопасно проверить, что приложить как evidence и какие действия требуют approval.
...
Лекция считается освоенной, если студент может объяснить `HTB lab access, VPN scope, enumeration notes, write-up evidence и product transfer boundary` своими словами, выполнить безопасный путь новичка, получить ожидаемый вывод, интерпретировать его и оформить evidence. Для темы `Занятие 51. HackTheBox Starting Point: все машины категории` минимальная сдача — оформить воспроизводимый артефакт по модели: HTB lab access, VPN scope, enumeration notes, write-up evidence и product transfer boundary, без нарушения ограничения: действия вне scope или без approval.
...
Минимальный эталонный артефакт для сдачи по теме `Занятие 51. HackTheBox Starting Point: все машины категории`:
