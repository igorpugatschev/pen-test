...
Книги курса использованы автором как источники для построения этой лекции, но не как обязательное домашнее чтение. Студенту не нужно искать недостающую теорию в отдельных файлах или внешних материалах: все понятия, команды, ограничения, безопасные примеры, ожидаемые результаты и критерии сдачи для темы `Занятие 50. TryHackMe (Jr Penetration Tester): 1-2 комнаты за занятие` должны быть понятны из текущего урока.
...
### 1. Предмет урока: Занятие 50. TryHackMe (Jr Penetration Tester): 1-2 комнаты за занятие

Этот блок отделяет легальную лабораторную практику от продуктового тестирования: lab дает техническую уверенность, а SDET переносит только безопасный процесс и test design. В этом уроке центральная модель: room scope, AttackBox, VPN, target IP, flags, write-up и lab boundary. Студент должен понять ее внутри самого Markdown-файла, без необходимости искать базовую теорию в книгах или внешних статьях.
...
### 4. SDET-практика: 1-2 комнаты за занятие
...
Минимальный результат урока: оформить lab-to-product transfer note. Если результат не достигается, студент должен уметь объяснить, чего не хватает: разрешения, среды, тестовых данных, изученного инструмента или безопасной лаборатории.
...
### 7. Типичные ошибки: 1-2 комнаты за занятие

Первая ошибка — выполнять практику до понимания модели `room scope, AttackBox, VPN, target IP, flags, write-up и lab boundary`. Это превращает обучение в копирование команд.
...
Студент должен выйти из урока с рабочим пониманием: что такое room scope, AttackBox, VPN, target IP, flags, write-up и lab boundary, как это безопасно проверить, что приложить как evidence и какие действия требуют approval.
...
Лекция считается освоенной, если студент может объяснить `room scope, AttackBox, VPN, target IP, flags, write-up и lab boundary` своими словами, выполнить безопасный путь новичка, получить ожидаемый вывод, интерпретировать его и оформить evidence. Для темы `Занятие 50. TryHackMe (Jr Penetration Tester): 1-2 комнаты за занятие` минимальная сдача — оформить lab-to-product transfer note, без нарушения ограничения: перенос CTF exploitation на Slider AI.
...
Минимальный эталонный артефакт для сдачи по теме `Занятие 50. TryHackMe (Jr Penetration Tester): 1-2 комнаты за занятие`:
