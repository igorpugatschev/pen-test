...
1. **WAF decision table**: заполните встроенную таблицу `input -> normalized input -> expected control -> logging expectation`.
2. **Bypass transcript**: разберите учебный пример, где encoding меняет вид payload в lab, и объясните, почему такую механику нельзя переносить на Slider AI без approval.
3. **DVWA High Security как углубление**: если лаборатория уже поднята, можно выполнить один lab-only пример и оформить его как `lab observation`, не как продуктовый finding.

WAF — это компенсирующий контроль, а не замена исправления. Для SDET/Security QA главный артефакт урока — проверяемое ожидание: canonicalization, server-side validation, logging, alerting и безопасный retest.
