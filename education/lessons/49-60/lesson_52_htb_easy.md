...
Книги курса использованы автором как источники для построения этой лекции, но не как обязательное домашнее чтение. Студенту не нужно искать недостающую теорию в отдельных файлах или внешних материалах: все понятия, команды, ограничения, безопасные примеры, ожидаемые результаты и критерии сдачи для темы `Занятие 52. HackTheBox (Easy): первая машина уровня Easy (Lame)` должны быть понятны из текущего урока.
...
### 3. Ключевые понятия: первая машина уровня Easy (Lame)

`Target` — конкретная разрешенная цель: локальная папка, localhost, учебная лаборатория, cloud lab или olddev-стенд.

`Mechanism` — технический механизм урока: HTB easy machine workflow, enumeration, foothold, privilege escalation as lab-only и report discipline. Его нужно объяснить до команды или инструмента.
...
### 4. SDET-практика: первая машина уровня Easy (Lame)
...
### 7. Типичные ошибки: первая машина уровня Easy (Lame)

Первая ошибка — выполнять практику до понимания модели `HTB easy machine workflow, enumeration, foothold, privilege escalation as lab-only и report discipline`. Это превращает обучение в копирование команд.
...
Студент должен выйти из урока с рабочим пониманием: что такое HTB easy machine workflow, enumeration, foothold, privilege escalation as lab-only и report discipline, как это безопасно проверить, что приложить как evidence и какие действия требуют approval.
...
Лекция считается освоенной, если студент может объяснить `HTB easy machine workflow, enumeration, foothold, privilege escalation as lab-only и report discipline` своими словами, выполнить безопасный путь новичка, получить ожидаемый вывод, интерпретировать его и оформить evidence. Для темы `Занятие 52. HackTheBox (Easy): первая машина уровня Easy (Lame)` минимальная сдача — оформить воспроизводимый артефакт по модели: HTB easy machine workflow, enumeration, foothold, privilege escalation as lab-only и report discipline, без нарушения ограничения: действия вне scope или без approval.
...
Минимальный эталонный артефакт для сдачи по теме `Занятие 52. HackTheBox (Easy): первая машина уровня Easy (Lame)`:
