...
Книги курса использованы автором как источники для построения этой лекции, но не как обязательное домашнее чтение. Студенту не нужно искать недостающую теорию в отдельных файлах или внешних материалах: все понятия, команды, ограничения, безопасные примеры, ожидаемые результаты и критерии сдачи для темы `Занятие 54. PortSwigger Academy: Web Security, 5-10 лаб по SQLi/XSS` должны быть понятны из текущего урока.
...
### 1. Предмет урока: Занятие 54. PortSwigger Academy: Web Security, 5-10 лаб по SQLi/XSS

Этот блок отделяет легальную лабораторную практику от продуктового тестирования: lab дает техническую уверенность, а SDET переносит только безопасный процесс и test design. В этом уроке центральная модель: граница SQL-запроса, параметризация, error-based/safe observation, ORDER BY в lab и prepared statements. Студент должен понять ее внутри самого Markdown-файла, без необходимости искать базовую теорию в книгах или внешних статьях.
...
### 4. SDET-практика: Web Security, 5-10 лаб по SQLi/XSS
...
Минимальный результат урока: отличить безопасную проверку валидации от SQLi exploitation. Если результат не достигается, студент должен уметь объяснить, чего не хватает: разрешения, среды, тестовых данных, изученного инструмента или безопасной лаборатории.
...
### 7. Типичные ошибки: Web Security, 5-10 лаб по SQLi/XSS

Первая ошибка — выполнять практику до понимания модели `граница SQL-запроса, параметризация, error-based/safe observation, ORDER BY в lab и prepared statements`. Это превращает обучение в копирование команд.
...
Студент должен выйти из урока с рабочим пониманием: что такое граница SQL-запроса, параметризация, error-based/safe observation, ORDER BY в lab и prepared statements, как это безопасно проверить, что приложить как evidence и какие действия требуют approval.
...
Лекция считается освоенной, если студент может объяснить `граница SQL-запроса, параметризация, error-based/safe observation, ORDER BY в lab и prepared statements` своими словами, выполнить безопасный путь новичка, получить ожидаемый вывод, интерпретировать его и оформить evidence. Для темы `Занятие 54. PortSwigger Academy: Web Security, 5-10 лаб по SQLi/XSS` минимальная сдача — отличить безопасную проверку валидации от SQLi exploitation, без нарушения ограничения: SQL payload на Slider AI без approval.
...
Минимальный эталонный артефакт для сдачи по теме `Занятие 54. PortSwigger Academy: Web Security, 5-10 лаб по SQLi/XSS`:
